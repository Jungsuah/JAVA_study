...
function validateForm() {
    ...
    if (stockId.length > 15) {
        alert("상품번호는 최대 15자리까지 입력할 수 있습니다.");
        stockIdInput.value = stockIdInput.value.substring(0, 15);
        stockIdInput.focus();
        return false;
    }
    ...
    if (stockName.length > 15) {
        alert("상품명은 최대 15자리까지 입력할 수 있습니다.");
        stockNameInput.value = stockNameInput.value.substring(0, 15);
        stockNameInput.focus();
        return false;
    }
    ...
    if (stockInventory.length > 9) {
        alert("재고현황은 최대 9자리까지 입력할 수 있습니다.");
        stockInventoryInput.value = stockInventoryInput.value.substring(0, 9);
        stockInventoryInput.focus();
        return false;
    }
    ...
    if (stockContent.length > 1000000) {
        alert("상품설명은 1,000,000글자를 초과할 수 없습니다.");
        stockContentInput.value = stockContentInput.value.substring(0, 1000000);
        stockContentInput.focus();
        return false;
    }
...
<form method="post" name="fm" enctype="multipart/form-data" action="writeAction.jsp" onsubmit="return validateForm()">
    <table class="tablestyle" border="1" cellspacing="0" cellpadding="5">
        ...
        <tr width="100" height="100">
            <td><b>상품설명</b></td>
            <td><textarea name="stockContent" class="textarea" cols="50" rows="5" maxlength="1000000"></textarea></td>
        </tr>
        <tr>
            <td><b>상품사진</b></td>
            <td>
