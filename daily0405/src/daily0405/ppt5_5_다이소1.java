...
public class ppt5_5_다이소1 {// 굴림체 10
   public static void main(String[] args) {
      ...
      // 이름배열 선언 및 초기화
      String[]  k27_itemname = { "퓨어에어 비말차단용마스크(최고급형)", "파머스픽 성주 참외", "꿈꾸는콩 촌두부 마을" };
      // 가격배열 선언 및 초기화
      int[]  k27_price = { 123456, 100, 1000 };
      // 코드배열 선언 및 초기화
      String[]  k27_itemcode = { "1031615", "11008152", "1020800" };
      // 갯수배열 선언 및 초기화
      int[]  k27_num = { 1, 1, 1 };
      // 과세 합계 선언 및 초기화
      int k27_val_total = 0;
      ...
      for (int i = 0; i <  k27_itemname.length; i++) {// 이름배열의 길이만큼 for문 돌기
         ...
         // i번째 가격 , 갯수 , 가격*갯수 출력
         System.out.printf("%s%8s%3d%8s\n", cc, k27_df.format( k27_price[i]),  k27_num[i], k27_df.format( k27_price[i] *  k27_num[i]));
      ...
      //최종 과세합계
      int k27_kk = k27_val_total - kopo_27_inetprice;

      System.out.printf("%17s %18s", "과세합계", k27_df.format(k27_kk));// 공간 지정 후 과세합계 출력
	         ...
	         try {
	            for (int i = 0; i < k27_len; i++) {// 14가 되기전까지 문자의 한칸이 2바이트면
	               if ((k27_by[i] & 0x80) == 0x80)// by[i]가 한글이면
	                  k27_count++; // k27_count+1
	            }
	            if ((k27_by[k27_len - 1] & 0x80) == 0x80 && (k27_count % 2) == 1) {// 14가 되기전까지 문자의 한칸이 1바이트면
	            	k27_len--; // 한칸 줄이고
