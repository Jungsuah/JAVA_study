...
public class freewifi_3_3 {
	public static void main(String[] args) throws Exception {
		...
		Statement k27_stmt = k27_conn.createStatement();
			...
			//service_provider가 SKT인 행을 freewifi 테이블에서 가져오는 쿼리 작성한다.
			...
			while (k27_rset.next()) {//원하는 결과값을 담은 객체에 내용이 없어질때까지 반복
				...
				System.out.printf("설치년월            : %s\n", k27_rset.getString(9));//getString을 사용하여 9번째 칼럼값 가져오기
				System.out.printf("소재지도로명주소    : %s\n", k27_rset.getString(10));//getString을 사용하여 10번째 칼럼값 가져오기
				System.out.printf("소재지지번주소      : %s\n", k27_rset.getString(11));//getString을 사용하여 11번째 칼럼값 가져오기
				System.out.printf("관리기관명          : %s\n", k27_rset.getString(12));//getString을 사용하여 12번째 칼럼값 가져오기
				System.out.printf("관리기관전화번호    : %s\n", k27_rset.getString(13));//getString을 사용하여 13번째 칼럼값 가져오기
				System.out.printf("위도               : %s\n", k27_rset.getString(14));//getString을 사용하여 14번째 칼럼값 가져오기
				System.out.printf("경도               : %s\n", k27_rset.getString(15));//getString을 사용하여 15번째 칼럼값 가져오기
				System.out.printf("데이터기준날짜      : %s\n", k27_rset.getString(16));//getString을 사용하여 16번째 칼럼값 가져오기
			}
