...
public class examtable_1 {
	public static void main(String[] args) throws ClassNotFoundException {
		...
		Connection conn = null;// MySQL 데이터베이스에 연결하기 위한 Connection 객체를 생성합니다.
		try {
		...
		Statement stmt = null;// SQL문을 실행하기 위한 Statement 객체를 생성합니다.
		try {
		...
		try {
			// Statement 객체를 닫습니다.
			stmt.close();//Open()한 만큼 close() 안하면 나중에 프로그램 박살된다
		} catch (SQLException e) {
			e.printStackTrace();//오류 발생시 오류를 알려준다.
		}
		try {
			// Connection 객체를 닫습니다.
			conn.close();//Open()한 만큼 close() 안하면 나중에 프로그램 박살난다
		} catch (SQLException e) {
			e.printStackTrace();//오류 발생시 오류를 알려준다.
		}
	}
