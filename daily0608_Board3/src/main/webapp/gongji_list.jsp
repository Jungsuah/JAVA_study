...
List<Gongji> gongjiList = gongjiDao.selectAll(pageNumber, cntPT);
int rootid = gongjiList.get(0).getId();
    ...
    <% for(Gongji gongji : gongjiList) { %>
        <tr>
            <td><%=gongji.getId()%></td>
             <td>
	            <% 
	            if(formattedDate.equals(gongji.getDate())){//오늘 작성된 게시글
	            String dashes = "";
	            ...
	            %>
            	<a href='gongji_view.jsp?id=<%=gongji.getId()%>&pageNumber=<%=c%>&viewcnt=<%=gongji.getViewcnt()%>'><%=dashes%><%=gongji.getTitle().replaceAll("<", "&lt;").replaceAll(">", "&gt;")%> [NEW]</a>
            	<% } else { %>
					    <% 
					    String dashes = "";
					    if(gongji.getRelevel() == 0) {
					    ...
					    %>
					    <a href='gongji_view.jsp?id=<%=gongji.getId()%>&pageNumber=<%=c%>&viewcnt=<%=gongji.getViewcnt()%>'><%=dashes%><%=gongji.getTitle().replaceAll("<", "&lt;").replaceAll(">", "&gt;")%> </a>
					<% } %>
        	</td>
