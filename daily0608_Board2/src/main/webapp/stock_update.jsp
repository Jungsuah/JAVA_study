...
function submitForm() {
    ...
    // 상품명 입력 제한: 공백이 아닌 문자, 문자+특수문자, 문자+숫자만 입력 가능
    var stockName = document.getElementsByName("stockName")[0].value;
    if (
        stockName.trim().length === 0 ||
        !/^[\da-zA-Z가-힣]+$/.test(stockName) ||
        /^\d+$/.test(stockName)
    ) {
        alert("상품명은 공백이 아닌 문자, 문자+특수문자, 문자+숫자만 입력할 수 있습니다.");
        return;
    }
